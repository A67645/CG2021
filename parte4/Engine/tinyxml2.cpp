...
#   include <cstdarg>
	...
	static inline int TIXML_SNPRINTF( char* positions, size_t size, const char* format, ... )
	{
		va_list va;
		va_start( va, format );
		const int result = vsnprintf_s( positions, size, _TRUNCATE, format, va );
		va_end( va );
		return result;
	}

	static inline int TIXML_VSNPRINTF( char* positions, size_t size, const char* format, va_list va )
	{
		const int result = vsnprintf_s( positions, size, _TRUNCATE, format, va );
		return result;
	}
...
XMLError XMLDocument::LoadFile( FILE* fp )
{
    ...
    if ( !LongFitsIntoSizeTMinusOne<>::Fits( filelength ) ) {
        // Cannot handle files which won't fit in positions together with null terminator
        SetError( XML_ERROR_FILE_READ_ERROR, 0, 0 );
        return _errorID;
    }
